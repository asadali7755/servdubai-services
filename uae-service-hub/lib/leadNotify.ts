...
/** Saves a lead to Supabase and emails the team — shared by the free "Request a Call" route and the paid-quote Stripe webhook. */
export async function notifyLead({ name, phone, service, message }: LeadFields) {
  const timestamp = new Date().toLocaleString('en-AE', { timeZone: 'Asia/Dubai' })

  const { error: dbError } = await supabase.from('contacts').insert([{ name, phone, service, message }])
  if (dbError) {
